...
// The exact correct getHTML string
...
// Fix the corrupted getHTML block entirely
code = code.replace(/function getHTML\([^]*?<\/script><\/body><\/html>`;\r?\n\}/m, getHtmlCode);

// Restore emojis in handleApiTool that were corrupted by ??
code = code.replace(/const icons=\{swagger:'.*?',rest:'.*?',graphql:'.*?',webhook:'.*?',ratelimit:'.*?',jwt:'.*?'\};/,
  "const icons={swagger:'📘',rest:'🔄',graphql:'🔷',webhook:'🪝',ratelimit:'⚡',jwt:'🔐'};");

// Fix the getSwaggerCode emoji and title
code = code.replace(/getHTML\('OpenAPI \/ Swagger', '.*?', 'Complete REST API specification', \[\{name: 'openapi\.yaml', icon: '.*?', code:/,
  "getHTML('OpenAPI / Swagger', '📘', 'Complete REST API specification', [{name: 'openapi.yaml', icon: '📝', code:");

// Fix graphql emoji
code = code.replace(/getHTML\('GraphQL Schema', '.*?', 'Complete GraphQL Types and Queries', \[\{name: 'schema\.graphql', icon: '.*?', code:/,
  "getHTML('GraphQL Schema', '🔷', 'Complete GraphQL Types and Queries', [{name: 'schema.graphql', icon: '🔷', code:");

// Fix rate limit emoji
code = code.replace(/getHTML\('Rate Limiter', '.*?', 'Token Bucket & Sliding Window logic', \[\{name: 'rate-limiter\.js', icon: '.*?', code:/,
  "getHTML('Rate Limiter', '⚡', 'Token Bucket & Sliding Window logic', [{name: 'rate-limiter.js', icon: '⚡', code:");

// Fix JWT emoji
code = code.replace(/getHTML\('JWT Auth Flow', '.*?', 'Login, Validation and Refresh Token system', \[\{name: 'jwt-auth\.js', icon: '.*?', code:/,
  "getHTML('JWT Auth Flow', '🔐', 'Login, Validation and Refresh Token system', [{name: 'jwt-auth.js', icon: '🔐', code:");

fs.writeFileSync(path, code);
console.log("Fixed getHTML and restored emojis successfully.");
