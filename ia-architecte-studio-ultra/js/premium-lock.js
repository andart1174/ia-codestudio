...
(function () {
  ...
  function getActiveLang() {
    var lang = 'en';
    if (typeof window.currentLang === 'string') lang = window.currentLang;
  ...
  window.showPaywallModal = function (featureName, customDesc) {
    ...
    if (isFr) {
      ...
      if (descEl) descEl.textContent = customDesc || 'La copie du code, les exports de fichiers, les enregistrements vidéo et les captures photo sont réservés aux membres Premium de IA Code Studio.';
      if (perksEl) perksEl.innerHTML =
        '<div><span class="check">✓</span> Copie illimitée du code HTML, CSS & JavaScript</div>' +
        '<div><span class="check">✓</span> Exportation de projets complets en ZIP & PWA</div>' +
        '<div><span class="check">✓</span> Enregistrements vidéo 60 FPS HD (WebM / MP4)</div>' +
        '<div><span class="check">✓</span> Captures photo HD, instantanés AR et maquettes</div>' +
        '<div><span class="check">✓</span> Déploiement en 1-clic sur le Cloud (Netlify)</div>';
      ...
      if (descEl) descEl.textContent = customDesc || 'Copying code, exporting files, video recordings, and photo captures are reserved for Premium members of IA Code Studio.';
      if (perksEl) perksEl.innerHTML =
        '<div><span class="check">✓</span> Unlimited HTML, CSS & JavaScript code copying</div>' +
        '<div><span class="check">✓</span> Full project ZIP & PWA standalone package exports</div>' +
        '<div><span class="check">✓</span> 60 FPS HD Video Screen Recordings (WebM / MP4)</div>' +
        '<div><span class="check">✓</span> HD photo captures, AR snapshots, and 3D mockups</div>' +
        '<div><span class="check">✓</span> 1-Click Cloud Deployment (Netlify)</div>';
