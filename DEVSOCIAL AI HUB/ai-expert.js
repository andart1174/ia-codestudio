...
(function() {
  ...
  const DOCS = {
    general: {
      ...
      fr: "Bonjour ! Je suis **Studio-AI**, votre assistant technique pour **IA CodeStudio**. Je pot vous aider pour tout ce qui concerne **IA ARCHITECTE** (notre constructeur de maison intelligent) sau **STUDIO 3D/4D PRO** (notre modeleur WebGL personnalisé). N'hésitez pas à poser une question en anglais ou en français !"
    },
    ...
    steam: {
      ...
      fr: "Pour personnaliser le **Système d'Échappement de Vapeur Steampunk** :\n1. Ouvrez le module **Chrono Steampunk** dans le volet latéral.\n2. Accédez à la section **Système d'échappement de vapeur**.\n3. Choisissez le type d'échappement : **Jet Périodique** (synchronisé avec le piston), **Jet Continu**, sau **Pulsation Cardiaque**.\n4. Choisissez la teinte de la vapeur à l'aide du sélecteur de couleurs.\n5. Lors de l'exportation sau de la previzualizare, la physique des particules Three.js (`THREE.Points`) est injectée automatiquement pour animer les évents."
    },
    ...
    paywall: {
      ...
      fr: "Le **Système de Verrouillage Premium** sécurise toutes les exportations. Les membres non-premium peuvent concevoir, personnaliser et prévisualiser les scènes dans le navigateur, mais les clics pe **Copier le code**, **Télécharger HTML**, **Pack ZIP** sau **Fusionner & Exporter** déclenchent le modal paywall."
    }
  };
  ...
  window.StudioAI = {
    generateResponse: function(userMessage, lang) {
      ...
      // Fallback
      if (lang === 'fr') {
        return "Je comprends votre question. Cependant, je suis spécifiquement entraîné pour vous guider sur **IA ARCHITECTE** et **STUDIO 3D/4D PRO**. Pourriez-vous préciser si vous souhaitez de l'aide concernant *l'avatar webcam*, *les particules de vapeur*, *le fond vert vidéo* sau *la fuziune des modules* ?";
      } else {
